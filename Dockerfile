...
RUN --mount=type=cache,target=/build/packages,sharing=locked \
    ...
    perl -i -pe 's/cmake\s+((\.\.\/)+)source\s+-DCMAKE/cmake $1source -DCMAKE_CXX_STANDARD=11 -DCMAKE_CXX_STANDARD_REQUIRED=ON "-DCMAKE_CXX_FLAGS=-include cstdint" -DCMAKE/' build-ffmpeg && \
